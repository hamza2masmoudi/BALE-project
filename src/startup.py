...
def check_environment() -> Tuple[bool, List[str]]:
    ...
    if not has_llm:
        warnings.append("⚠️  No LLM configured - analysis will fail (set LOCAL_LLM_ENDPOINT or MISTRAL_API_KEY)")
...
def check_dependencies() -> Tuple[bool, List[str]]:
    ...
    for package, name in optional_packages:
        try:
            ...
            missing.append(f"⚠️  {name} ({package}) not installed (optional)")
...
def check_database() -> Tuple[bool, str]:
    ...
    if not db_url:
        return False, "⚠️  DATABASE_URL not set"
    ...
    try:
        ...
        return False, "⚠️  Database module not available"
    except Exception as e:
...
def run_startup_checks(exit_on_critical: bool = False) -> bool:
    ...
    if all_ok:
        ...
        print("⚠️  Some checks failed - BALE may have limited functionality")
    print("="*60 + "\n")
