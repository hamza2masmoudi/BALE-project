        ...
        for change in self.regime_changes:
            ...
            if contract_dt < change_date <= today:
                ...
                matching_jurisdictions = [
                    j for j in change["jurisdictions"] 
                    if j == jurisdiction_enum or (
                        jurisdiction_enum == Jurisdiction.US_FEDERAL and 
                        str(j.value).startswith("US")
                    )
                ]
...
class DeonticStrainDetector:
    ...
    - P(action): Permitted to do action  
    - F(action): Forbidden to do action
    ...
    def _check_conflict(
        ...
        if s1.agent == s2.agent:
            ...
            if (s1.modality == ObligationType.POSITIVE and 
                s2.modality == ObligationType.NEGATIVE):
