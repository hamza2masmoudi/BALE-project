...
CONFLICTS = "conflicts" # Clauses contradict each other
DEPENDS_ON = "depends_on" # Clause A requires Clause B to exist
LIMITS = "limits" # Clause A restricts the scope of Clause B
SUPPLEMENTS = "supplements" # Clause A adds detail to Clause B
REFERENCES = "references" # Clause A mentions Clause B's concept
# ==================== KNOWN RELATIONSHIPS ====================
# These are the legal relationships between clause types that a lawyer would know.
# This is the "knowledge" in our system — extracted from legal doctrine.
# Format: (source_type, target_type, edge_type, description, conflict_severity)
...
# What clauses a well-drafted contract of each type should have.
...
risk_weight: float # Base risk for this type
category: str # e.g., "liability"
is_present: bool = True # False for "expected but missing" nodes
party_obligations: Dict[str, int] = field(default_factory=dict) # party -> count of obligations
@dataclass class ClauseEdge:
"""An edge in the contract graph representing a relationship."""
source_id: str
target_id: str
edge_type: EdgeType
description: str
severity: float # 0-1, how serious this relationship is
is_satisfied: bool = True # For dependencies: is the target present?
@dataclass
class GraphAnalysis:
...
# Aggregate scores
structural_risk: float # 0-100, based on gaps and conflicts
conflict_count: int
dependency_gap_count: int
completeness_score: float # 0-1, how complete is this contract
...
The graph reveals:
1. Clauses that CONFLICT with each other
2. Clauses that DEPEND on other clauses that don't exist
3. Standard clauses that are MISSING for this contract type
"""
def __init__(self):
self.nodes: Dict[str, ClauseNode] = {}
self.edges: List[ClauseEdge] = []
self.clause_type_to_node: Dict[str, str] = {} # type -> node_id
...
Only creates edges where both (or at least one) clause exists.
"""
...
# Only flag conflict if BOTH clauses exist
...
description=desc,
severity=severity,
is_satisfied=True # Both present = conflict exists
))
elif edge_type == EdgeType.DEPENDS_ON:
# Flag dependency gap if source exists but target doesn't
...
description=desc,
severity=severity,
is_satisfied=False # Dependency not met!
))
...
# Higher prevalence = more concerning absence
risk = int(prevalence * 40) # Max 40 risk points for missing clause
missing.append({
"clause_type": clause_type,
"expected_prevalence": prevalence,
"risk_contribution": risk,
"recommendation": f"Consider adding a {clause_type.replace('_', ' ')} clause. "
...
Returns conflicts, missing dependencies, and structural risk score.
"""
# Build edges if not already done
if not self.edges:
self.build_edges()
# Find conflicts
conflicts = []
...
missing_risk = sum(m["risk_contribution"] for m in missing_expected[:5]) # Top 5 missing
...
- id: str
- text: str - clause_type: str
- confidence: float
- risk_weight: float (optional)
- category: str (optional)
contract_type: Type of contract for gap analysis
Returns:
