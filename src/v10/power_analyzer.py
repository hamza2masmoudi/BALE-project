...
# ==================== OBLIGATION PATTERNS ====================
# Regex patterns that indicate an obligation directed at a specific party
OBLIGATION_MARKERS = [
...
power_score: float # 0 = balanced, 100 = completely one-sided
dominant_party: str # Name of the party with more power
burdened_party: str # Name of the party with more obligations
asymmetric_clauses: List[Dict[str, Any]] # Clauses that are one-sided
...
if (role and len(role) < 30 and role.lower() not in non_parties and role not in parties):
parties.append(role)
# Fallback: look for common party references
if len(parties) < 2:
common_roles = [
...
return parties[:2] # Use first two parties
def analyze(
self,
...
# Determine which party is referenced
p0_mentioned = bool(re.search(r'\b' + re.escape(parties[0]) + r'\b', text, re.IGNORECASE))
p1_mentioned = bool(re.search(r'\b' + re.escape(parties[1]) + r'\b', text, re.IGNORECASE))
# Assign obligations and protections to parties
if p0_mentioned and not p1_mentioned:
# Only first party mentioned — they bear the obligations
...
summary = f"Moderate power imbalance favoring {dominant}. {len(asymmetric)} one-sided clauses detected."
else:
summary = f"Significant power imbalance. {dominant} holds dominant position with {len(asymmetric)} asymmetric clauses. {burdened} bears disproportionate obligations."
