...
clause_type: str
clause_id: str
dispute_probability: float # 0-1
severity: str # LOW, MEDIUM, HIGH, CRITICAL
reason: str # Why this clause will be disputed
category: str # conflict, gap, power, ambiguity
...
overall_dispute_risk: float # 0-100
top_3_risks: List[str] # Human-readable summary of top 3 risks
dispute_count_prediction: str # "Low (0-1)", "Medium (2-4)", "High (5+)"
def to_dict(self) -> Dict:
return {
...
category="conflict",
recommendation=f"Reconcile {conflict['clause_a']} with {conflict['clause_b']}. "
...
dispute_probability=prob,
severity=self._severity(prob),
reason=f"{dep['clause_has']} depends on {dep['clause_needs']}, but {dep['clause_needs']} is missing. {dep['description']}",
category="gap",
recommendation=f"Add a {dep['clause_needs'].replace('_', ' ')} clause to support the {dep['clause_has'].replace('_', ' ')} provision."
))
...
dispute_probability=prob,
severity=self._severity(prob),
reason=f"One-sided clause favoring {asym.get('favors', 'unknown')}. "
f"Triggers: {', '.join(asym.get('triggers', [])[:2])}",
category="power",
recommendation=f"Negotiate more balanced terms in {asym['clause_type'].replace('_', ' ')}."
))
...
dispute_probability=prob,
severity=self._severity(prob),
reason=f"Expected clause '{missing['clause_type'].replace('_', ' ')}' is missing. "
f"Present in {int(missing['expected_prevalence'] * 100)}% of similar contracts.",
category="gap",
...
# Overall risk
if hotspots:
weighted_risk = sum(h.dispute_probability * (1 if h.severity == "CRITICAL" else 0.7) for h in hotspots[:5])
